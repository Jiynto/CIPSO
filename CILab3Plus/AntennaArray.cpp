...
AntennaArray::AntennaArray(unsigned int n_ant, double steering_ang)
  : n_antennae(n_ant), steering_angle(steering_ang), MAX_APERTURE((double)n_ant/2)
{}

double AntennaArray::getAntennae(void)
{
    return n_antennae;
}
...
bool AntennaArray::is_valid(std::vector<double> design)
{
  ...
  for(size_t i = 0;i<design.size()-1;++i)
    if(design[i] < 0 || design[i] > MAX_APERTURE )
      return false;
